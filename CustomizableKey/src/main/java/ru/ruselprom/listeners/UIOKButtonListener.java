...
public class UIOKButtonListener extends DefaultPushButtonListener {
	...
	public void OnActivate(PushButton handle) throws jxthrowable {													
		try {
			...
			implementation = Core.properties.getProperty("key_template");
			...
			if (panel1.GetTextValue().equals("")) {
				panel1.SetTextValue(Core.dtf.format(Core.now));
				return;
			}
	...
	public static void chooseDimensionsAndBuild() throws jxthrowable {
		...
		if (Core.keyLength == 0 || Core.keyWidth == 0 || Core.keyHeight == 0) {
			session.UIShowMessageDialog("Некоторые размеры не были выбраны!", null);
			return;
		}
		
		double chamfer = Core.loadChamfer(Core.keyWidth, Core.keyHeight);
		
		session.ChangeDirectory(Core.copiesFolder);
		
		buildModelAndDrawing(session, Core.keyLength, Core.keyWidth,  Core.keyHeight, chamfer, type);
	}
	...
	public static void calculateByKnownPressureAndBuild() throws jxthrowable {
		...
		double chamfer = Core.loadChamfer(keyWidth, keyHeight);
		ResultSet result;
		String SQLstring;
		double groove = 0;
		try {
			...
			result = Core.executeQuery(SQLstring);
			result.next();
		...
		try {
			result = Core.executeQuery("SELECT * FROM dbo.GOSTTable WHERE (width = " + Integer.toString(keyWidth) + " AND height = " + Integer.toString(keyHeight) + ");");
			result.next();
			if (calculatedLength >= result.getInt("lengthMin") && calculatedLength <= result.getInt("lengthMax")) {
				...
				ResultSet resultLengths = Core.executeQuery("SELECT length FROM dbo.lengths WHERE ((Select lengthMin FROM dbo.GOSTtable Where " + subString + ") <= length AND length <= (Select lengthMax FROM dbo.GOSTtable Where " + subString + "));");
				...
				// no gost variant
				//ResultSet resultLengths = Core.executeQuery("SELECT length FROM dbo.lengths WHERE (length > " + calculatedLength + ");");
				//resultLengths.next();
				//keyLength = resultLengths.getInt("length");
				// gost variant
				session.UIShowMessageDialog("Расчетная длина меньше допустимой. Выбрана минимальная длина для выбранного сечения", null);
				String subString = "(width = " + Integer.toString(keyWidth) + " AND height = " + Integer.toString(keyHeight) + ")";
				ResultSet resultLengths = Core.executeQuery("SELECT length FROM dbo.lengths WHERE ((Select lengthMin FROM dbo.GOSTtable Where " + subString + ") <= length AND length <= (Select lengthMax FROM dbo.GOSTtable Where " + subString + "));");
				resultLengths.next();
				keyLength = resultLengths.getInt("length");
			} else if (calculatedLength > result.getInt("lengthMax")) {
				session.UIShowMessageDialog("Расчетная длина больше допустимой для выбранного сечения. Выбрана первая длина, большая расчетной", null);
				ResultSet resultLengths = Core.executeQuery("SELECT length FROM dbo.lengths WHERE (length > " + calculatedLength + ");");
				resultLengths.next();
				keyLength = resultLengths.getInt("length");
			}
	...
	public static void buildModelAndDrawing(Session session, int length, int width, int height, double chamfer, int type) throws jxthrowable {
		Model partModel = loadModel(newModelName, Core.textFolder + implementation + ".prt");
		...
		Model drawingModel = loadModel(newModelName + "_", Core.copiesFolder + newModelName + ".drw");
